...
function App() {
  ...
  function handleAction(action) {
    ...
    if (action?.type === 'integration') return runMutation(`/api/integrations/${action.id}`, jsonOptions('PATCH', { status: action.status }), 'Integration status updated')
    if (action?.type === 'delete-reseller') return runMutation(`/api/resellers/${action.id}`, { method: 'DELETE' }, 'Reseller account deleted')
    if (action?.type === 'delete-user') return runMutation(`/api/users/${action.id}`, { method: 'DELETE' }, 'Subscriber account deleted')
    if (action?.type === 'delete-content') return runMutation(`/api/content/${action.id}`, { method: 'DELETE' }, 'Content item deleted')
    if (action?.type === 'delete-server') return runMutation(`/api/servers/${action.id}`, { method: 'DELETE' }, 'Server deleted')
    if (action?.type === 'delete-source') return runMutation(`/api/sources/${action.id}`, { method: 'DELETE' }, 'Stream source deleted')
    if (action?.type === 'delete-category') return runMutation(`/api/categories/${action.id}`, { method: 'DELETE' }, 'Category deleted')
    if (action?.type === 'delete-epg') return runMutation(`/api/epg/${action.id}`, { method: 'DELETE' }, 'EPG schedule deleted')
...
function AnalyticsView({ summary = {}, users = [], content = [] }) {
  const activeUsers = users.filter((item) => item.status === 'active').length
  const bars = [0.25, 0.37, 0.2, 0.55, 0.43, 0.7, 0.61].map((value, index) => Math.max(0.08, Math.min(1, value + ((activeUsers + content.length + index) % 4) * 0.05)))
  return <div className="analytics-layout"><div className="panel analytics-main"><div className="panel-heading"><div><span className="section-kicker">AUDIENCE OVERVIEW</span><h2>Subscriber growth</h2></div><span className="muted-label">{activeUsers} active subscribers · {content.length} content items</span></div><div className="analytics-empty"><div className="bar-placeholder">{bars.map((height, index) => <i key={index} style={{ height: `${height * 100}%` }} />)}</div><p>{activeUsers || content.length ? `Tracking ${activeUsers} active subscribers across ${content.length} content items.` : 'Analytics will populate as your network receives traffic.'}</p></div></div><div className="panel insight-card"><div className="metric-icon cyan"><Zap size={18} /></div><span className="section-kicker">INSIGHT</span><h3>{summary.resellerAccounts ? 'Partner network is ready' : 'Build your first audience'}</h3><p>{summary.resellerAccounts ? `${summary.resellerAccounts} reseller accounts are available for distribution.` : 'Add subscribers and content to unlock network insights.'}</p></div></div>
}
...
function UsersView({ users, onAction, onEdit, onDelete }) {
  const [query, setQuery] = useState('')
  const [statusFilter, setStatusFilter] = useState('all')
